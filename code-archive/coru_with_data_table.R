...
iv <- c("lwage")
dv <- c("exp", "expsq", "wks", "bluecol", "ind", "south", "smsa", "married", "union")
gid <- c("id")
tid <- c("time")
...
for (idx in 1:n) {
  ...
  x <- data.matrix(panel.set[id == idx][, ..dv])
  ...
  y <- data.matrix(panel.set[id == idx][, ..iv])
  stacked.y <- c()
  for (timespan in 1:(Tt - 1)) {
...
# Plotting ====
{
  ...
  for (row in 1:k) {
    ...
    abline(h = wages.fe$coefficients[row], lty = 3)
    abline(h = 0, lty = 2)
    arrows(
      x0 = 1:(Tt - 1), 
